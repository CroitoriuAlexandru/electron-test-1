...
export default function App() {
      ...
      window.api.db.getToken().then((result) => {
        ...
        fetch(endpoints.company.get_organigram_info, {
          ...
          .then((data) => {
            ...
            let nods = drawChart(data.ceo, data.departments, data.employees)
            for (const x in nods) {
              window.api.printTest(nods[x]);
              chart.add(nods[x]);
            }
            // window.api.printTest(chart.nodes);
            // console.log("laksjhflkajsdhfljkahdflajkhsdlfjhasdlfjh", data);
            // rez.push(data.ceo);
            // rez.push(data.departments);
            // rez.push(data.employees);
  ...
  const drawChart = (ceo, deps, employees) => {
    let nodes = [];
  ...
  // initializeChart();
  return (
    <div class="text-white border bg-neutral-200 h-[100vh] flex transparent ">
      <button
        onClick={() => {
          window.api.printTest(accessToken);
          initializeChart();
