...
export default function App() {
  ...
  const fetchOrganigramInfo = () => {
    console.log("fetch organigram info ")
    window.api.db.getToken().then((result) => {
      ...
      fetch(endpoints.company.get_organigram_info, {
        headers: {
          Authorization: `Bearer ${result.dataValues.access}`,
        },
      })
      .then((res) => res.json())
      .then( async (data) => {
        let nods = await drawChart(data.ceo, data.departments, data.employees);
        for (const x in nods) {
          chart.add(nods[x]);
        }
      })
      .catch((error) => {
        window.api.printTest("Error fetching organigram info:", error);
      });
    })
    .catch((error) => {
      window.api.printTest("Error fetching token:", error);
    });
  };
  const getLastId = () => {
    let rez = -1;
  ...
  const drawChart = async (ceo, deps, employees) => {
    let nodes = [];
    let data_ceo = {
      id: 1,
    ...
    console.log("lkajsdhfkjasdhgfkjahsgdfkas", nodes)
    return nodes;
  };
  ...
  const initializeChart = async () => {
    ...
    chart.onNodeClick(function (args) {
      ...
      if (args.event.target.attributes.googlebutton !== undefined) {
        ...
        axios
          .get(endpoints.company.get_user_employees_from_google, {
            headers: {
  ...
  const set_node_info = (employee_id, department_id, supervizer_id) => {
    console.log("set node info");//, employee_id-1000, department_id, supervizer_id-1000);
    try {
      fetch(endpoints.company.set_employee_department_and_supervizer, {
        method: "POST",
        headers: {
        ...
        .then((data) => {
          console.log("Response from set_employee_department:", data);
          setRedrawChart(!redrawChart);
        })
        .catch((error) => {
  ...
  // useEffect(() => {
  //   window.api.topWindow.printTest(`access token: ${accessToken}`);
  ...
  return (
    <div className="h-[100vh] flex steps-background">
      <button onClick={() => {window.api.printTest(accessToken)}} class="bg-gray-500 border">Test</button>
      <div
